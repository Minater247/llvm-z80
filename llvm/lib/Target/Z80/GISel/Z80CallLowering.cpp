...
struct Z80OutgoingValueHandler : public CallLowering::OutgoingValueHandler {
  ...
  void finish(CallingConv::ID CallConv) {
    if (CallConv == CallingConv::Z80_TIFlags) {
...
struct CallArgHandler : public Z80OutgoingValueHandler {
  ...
  void finish(CallingConv::ID CallConv, uint64_t StackSize) {
    FrameSize = StackSize;
    Z80OutgoingValueHandler::finish(CallConv);
    MIRBuilder.setInsertPt(MIRBuilder.getMBB(), RegCopies);
  }
...
struct FormalArgHandler : public Z80IncomingValueHandler {
  ...
  void finish(uint64_t StackSize, bool IsVarArg) {
    MachineFunction &MF = MIRBuilder.getMF();
    auto &FuncInfo = *MF.getInfo<Z80MachineFunctionInfo>();
    FuncInfo.setArgFrameSize(StackSize);
    if (IsVarArg) {
      int FrameIdx = MF.getFrameInfo().CreateFixedObject(1, StackSize, true);
...
bool Z80CallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
  ...
  Handler.finish(Assigner.StackSize, F.isVarArg());
