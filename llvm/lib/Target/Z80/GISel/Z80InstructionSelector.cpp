...
bool Z80InstructionSelector::selectSExt(MachineInstr &I,
  ...
  Fill->findRegisterUseOperand(FillReg, &TRI)->setIsUndef();
        ...
        if (LoadMI->getOpcode() == TargetOpcode::G_LOAD &&
          ...
          if (auto ImmConst = getIConstantVRegVal(ImmReg, MRI)) {
            ...
            case TargetOpcode::G_FSHL:
              ...
              if (ImmConst->isOne()) {
                RMWOps.clear();
                RMWOps.push_back(Z80::RLC8p);
                RMWOps.push_back(Z80::RLC8o);
              }
              else if (*ImmConst == 7) {
                RMWOps.clear();
                RMWOps.push_back(Z80::RRC8p);
                RMWOps.push_back(Z80::RRC8o);
              }
              break;
            case TargetOpcode::G_FSHR:
              if (ValMI->getOperand(2).getReg() != LoadReg)
            ...
            case TargetOpcode::G_SHL:
              if (ImmConst->isOne()) {
                RMWOps.clear();
                RMWOps.push_back(Z80::SLA8p);
                RMWOps.push_back(Z80::SLA8o);
              }
              break;
            case TargetOpcode::G_ASHR:
              if (ImmConst->isOne()) {
                RMWOps.clear();
                RMWOps.push_back(Z80::SRA8p);
                RMWOps.push_back(Z80::SRA8o);
              }
              break;
            case TargetOpcode::G_LSHR:
              if (ImmConst->isOne()) {
                RMWOps.clear();
                RMWOps.push_back(Z80::SRL8p);
                RMWOps.push_back(Z80::SRL8o);
              }
              break;
            case TargetOpcode::G_AND: {
              ...
              if (Bit >= 0) {
                RMWOps.clear();
                RMWOps.push_back(Z80::RES8pb);
                RMWOps.push_back(Z80::RES8ob);
                RMWOps.push_back(unsigned(Bit));
              }
              break;
            }
            case TargetOpcode::G_OR: {
              ...
              if (Bit >= 0) {
                RMWOps.clear();
                RMWOps.push_back(Z80::SET8pb);
                RMWOps.push_back(Z80::SET8ob);
                RMWOps.push_back(unsigned(Bit));
              }
              break;
            }
            case TargetOpcode::G_ADD:
              if (ImmConst->isOne()) {
                RMWOps.clear();
                RMWOps.push_back(Z80::INC8p);
                RMWOps.push_back(Z80::INC8o);
              } else if (ImmConst->isAllOnes()) {
                RMWOps.clear();
                RMWOps.push_back(Z80::DEC8p);
                RMWOps.push_back(Z80::DEC8o);
              }
              break;
            }
          }
...
bool Z80InstructionSelector::selectMergeValues(MachineInstr &I,
  ...
  case 24: {
    ...
    if (mi_match(UpReg, MRI, m_GAShr(m_Reg(TmpReg), m_SpecificICst(7)))) {
      ...
      SbcI->findRegisterUseOperand(Z80::UHL, &TRI)->setIsUndef();
...
Z80InstructionSelector::foldCompare(MachineInstr &I, MachineIRBuilder &MIB,
  ...
  SmallVector<SrcOp, 2> Ops;
  Ops.push_back(RHSReg);
  if (ConstRHS) {
    if (!ConstRHS->Value && (CC == Z80::COND_Z || CC == Z80::COND_NZ)) {
      if (OpSize == 8) {
        ...
        if (mi_match(LHSReg, MRI,
            ...
            isPowerOf2_32(static_cast<uint8_t>(Mask))) {
          uint8_t Mask8 = static_cast<uint8_t>(Mask);
          ...
          if ((LoadMI = MRI.getVRegDef(SrcReg)) && LoadMI->getOpcode() == TargetOpcode::G_LOAD &&
            ...
            BitI.add(LoadMI->getOperand(1));
            unsigned BitIdx = llvm::countr_zero(static_cast<unsigned>(Mask8));
            BitI.addImm(BitIdx);
            ...
            unsigned BitIdx = llvm::countr_zero(static_cast<unsigned>(Mask8));
            Ops.push_back(static_cast<uint64_t>(BitIdx));
          }
        } else {
          Opc = Z80::OR8ar;
          Ops.clear();
      ...
      Opc = Z80::CP8ai;
      Ops.clear();
      Ops.push_back(ConstRHS->Value.getSExtValue());
    } else {
      auto LDI =
...
bool Z80InstructionSelector::selectShift(MachineInstr &I,
  ...
  if (Amt->Value == Ty.getSizeInBits() - 1) {
    ...
    if (Ty == LLT::scalar(8)) {
      SbcI->findRegisterUseOperand(Reg, &TRI)->setIsUndef();
...
bool Z80InstructionSelector::selectInlineAsm(MachineInstr &I,
  ...
  for (unsigned Idx :
    ...
    if (const auto *RC = I.getRegClassConstraint(Idx, &TII, &TRI))
      Reg = constrainOperandRegClass(*MF, TRI, MRI, TII, RBI, I, *RC, MO);
  }
  return true;
}
