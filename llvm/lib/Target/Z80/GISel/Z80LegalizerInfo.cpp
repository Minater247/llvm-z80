...
Z80LegalizerInfo::Z80LegalizerInfo(const Z80Subtarget &STI,
  ...
  for (int AddrSpace = 0; AddrSpace != static_cast<int>(p.size()); ++AddrSpace)
    p[AddrSpace] = LLT::pointer(AddrSpace, DL.getPointerSizeInBits(AddrSpace));
  LLT s1 = LLT::scalar(1);
  LLT s8 = LLT::scalar(8);
  ...
  for (unsigned MemOp : {G_LOAD, G_STORE})
    getLegacyLegalizerInfo().setLegalizeScalarToDifferentSizeStrategy(
      MemOp, 0, LegacyLegalizerInfo::widenToLargerTypesAndNarrowToLargest);
...
bool Z80LegalizerInfo::legalizeCustom(LegalizerHelper &Helper, MachineInstr &MI,
  ...
  LegalizerHelper::LegalizeResult Result = LegalizerHelper::UnableToLegalize;
  switch (MI.getOpcode()) {
  default:
    // No idea what to do.
    Result = LegalizerHelper::UnableToLegalize;
    break;
  case G_ADD:
  case G_SUB:
    Result = legalizeAddSub(Helper, MI, LocObserver);
    break;
  case G_AND:
  case G_OR:
  case G_XOR:
  case G_PTRMASK:
    Result = legalizeBitwise(Helper, MI, LocObserver);
    break;
  case G_EXTRACT:
  case G_INSERT:
    Result = legalizeExtractInsert(Helper, MI);
    break;
  case G_FCONSTANT:
    Result = legalizeFConstant(Helper, MI);
    break;
  case G_VASTART:
  ...
  case G_LSHR:
  case G_ASHR:
    Result = legalizeShift(Helper, MI, LocObserver);
    break;
  case G_FSHL:
  case G_FSHR:
  case G_ROTR:
  case G_ROTL:
    Result = legalizeFunnelShift(Helper, MI);
    break;
  case G_ICMP:
  case G_FCMP:
    Result = legalizeCompare(Helper, MI, LocObserver);
    break;
  case G_UMULO:
    Result = legalizeMultiplyWithOverflow(Helper, MI);
    break;
  case G_SMULFIX:
  case G_UMULFIX:
  case G_SMULFIXSAT:
  case G_UMULFIXSAT:
    Result = legalizeFixedMultiply(Helper, MI);
    break;
  case G_SDIVFIX:
  case G_UDIVFIX:
  case G_SDIVFIXSAT:
  case G_UDIVFIXSAT:
    Result = legalizeFixedDivide(Helper, MI);
    break;
  case G_FCANONICALIZE:
    Result = legalizeFCanonicalize(Helper, MI);
    break;
  case G_CTLZ:
    Result = legalizeCtlz(Helper, MI, LocObserver);
    break;
  case G_MEMCPY:
  ...
  case G_MEMSET:
  case G_BZERO:
    Result = legalizeMemIntrinsic(Helper, MI, LocObserver);
    break;
  }
...
Z80LegalizerInfo::legalizeAddSub(LegalizerHelper &Helper, MachineInstr &MI,
  ...
  if (mi_match(MI, MRI, m_Neg(m_Reg(LHSReg)))) {
    ...
    CallLowering::ArgInfo ResultInfo({DstReg}, Ty, 0);
    SmallVector<CallLowering::ArgInfo, 1> ArgInfos;
    ArgInfos.emplace_back(ArrayRef<Register>{LHSReg}, Ty, 0);
    auto Result = createLibcall(Helper.MIRBuilder, Libcall, ResultInfo,
                  ArgInfos, LocObserver, &MI);
    MI.eraseFromParent();
    return Result;
  }
  if (LegalSize)
...
Z80LegalizerInfo::legalizeBitwise(LegalizerHelper &Helper, MachineInstr &MI,
  ...
  if (MI.getOpcode() == G_PTRMASK) {
    ...
    Type *Ty = IntegerType::get(Ctx, Size);
    CallLowering::ArgInfo ResultInfo({DstReg}, Ty, 0);
    SmallVector<CallLowering::ArgInfo, 2> ArgInfos;
    ArgInfos.emplace_back(ArrayRef<Register>{MI.getOperand(1).getReg()}, Ty,
                0);
    ArgInfos.emplace_back(ArrayRef<Register>{MI.getOperand(2).getReg()}, Ty,
                1);
    auto Result = createLibcall(Helper.MIRBuilder, Libcall, ResultInfo,
                  ArgInfos, LocObserver, &MI);
    MI.eraseFromParent();
...
Z80LegalizerInfo::legalizeVAStart(LegalizerHelper &Helper,
  ...
  LLT p0 = LLT::pointer(0, MF.getDataLayout().getPointerSizeInBits(0));
...
Z80LegalizerInfo::legalizeCompare(LegalizerHelper &Helper,
  ...
  if (!IsConst) {
    ...
    ArgInfos.emplace_back(ArrayRef<Register>{LHSReg}, Ty, 0);
    if (!ZeroRHS)
      ArgInfos.emplace_back(ArrayRef<Register>{RHSReg}, Ty, 1);
    auto Result = createLibcall(MIRBuilder, Libcall, FlagsArg, ArgInfos,
                                LocObserver, &MI);
...
Z80LegalizerInfo::legalizeCtlz(LegalizerHelper &Helper, MachineInstr &MI,
  ...
  CallLowering::ArgInfo ResultInfo({DstReg},
                                   IntegerType::get(Ctx, DstSize), 0);
  SmallVector<CallLowering::ArgInfo, 1> ArgInfos;
  ArgInfos.emplace_back(ArrayRef<Register>{SrcReg},
                        IntegerType::get(Ctx, SrcSize), 0);
  auto Result = createLibcall(MIRBuilder, Libcall, ResultInfo, ArgInfos,
                              LocObserver, &MI);
  MI.eraseFromParent();
  return Result;
}
...
bool Z80LegalizerInfo::legalizeIntrinsic(LegalizerHelper &Helper,
  ...
  case Intrinsic::trap: {
    ...
    Info.OrigRet = CallLowering::ArgInfo(ArrayRef<Register>{},
                       Type::getVoidTy(Ctx),
                       CallLowering::ArgInfo::NoArgIndex);
