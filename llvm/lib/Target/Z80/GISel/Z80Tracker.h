...
namespace llvm {
  ...
  public:
    ...
    private:
      ...
      std::optional<std::pair<Register, int64_t>> regOff;
    ...
    public:
      ...
      bool isImm() { return immediate.has_value(); }
      ...
      bool isRegOff() { return regOff.has_value(); }
      ...
      static std::string getName(const TargetRegisterInfo *TRI, Register Reg) {
        if (Reg.isVirtual()) {
          char buf[16];
          snprintf(buf, sizeof(buf), "%%%u", Reg.virtRegIndex());
          return buf;
        } else {
  ...
  public:
    ...
    std::optional<int64_t> extractImmediate(MachineOperand &MO);
