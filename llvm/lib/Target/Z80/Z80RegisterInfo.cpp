...
Z80RegisterInfo::getLargestLegalSuperClass(const TargetRegisterClass *RC,
  ...
  auto IsLegalSizeClass = [](const TargetRegisterClass *Cls) {
    switch (Cls->getID()) {
  ...
  if (IsLegalSizeClass(RC))
    return RC;

  for (unsigned SuperID : RC->superclasses()) {
    const TargetRegisterClass *Super = getRegClass(SuperID);
    if (IsLegalSizeClass(Super))
      return Super;
  }
...
bool Z80RegisterInfo::saveScavengerRegister(MachineBasicBlock &MBB,
  ...
  const TargetRegisterClass *UseRC = RC;
  if (!UseRC) {
    if (!Reg.isPhysical())
      return false;
    UseRC = getMinimalPhysRegClass(Reg);
  }
  if (!UseRC)
    return false;

  unsigned RegSize = getRegSizeInBits(*UseRC);
  if ((RegSize == 16 && Is24Bit) || (RegSize == 24 && !Is24Bit) ||
      (RegSize != 16 && RegSize != 24))
    return false;

  if (Reg == Z80::AF)
    BuildMI(MBB, MI, DL, TII.get(RegSize == 24 ? Z80::PUSH24AF : Z80::PUSH16AF));
  else
    BuildMI(MBB, MI, DL, TII.get(RegSize == 24 ? Z80::PUSH24r : Z80::PUSH16r))
        .addReg(Reg);
  ...
  if (Reg == Z80::AF)
    BuildMI(MBB, UseMI, DL, TII.get(RegSize == 24 ? Z80::POP24AF : Z80::POP16AF));
  else
    BuildMI(MBB, UseMI, DL, TII.get(RegSize == 24 ? Z80::POP24r : Z80::POP16r),
            Reg);
  return true;
}
