...
static void markUseUndef(MachineInstr &MI, Register Reg,
                         const TargetRegisterInfo &TRI) {
  if (MachineOperand *MO = MI.findRegisterUseOperand(Reg, &TRI))
    MO->setIsUndef();
}

static void markUseKill(MachineInstr &MI, Register Reg,
                        const TargetRegisterInfo &TRI) {
  if (MachineOperand *MO = MI.findRegisterUseOperand(Reg, &TRI))
    MO->setIsKill();
}

static void markDefDead(MachineInstr &MI, Register Reg,
                        const TargetRegisterInfo &TRI) {
  if (MachineOperand *MO = MI.findRegisterDefOperand(Reg, &TRI))
    MO->setIsDead();
}
...
unsigned Z80InstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  ...
  auto TSFlags = Desc.TSFlags;
  ArrayRef<MCOperandInfo> OpInfos = Desc.operands();
  ...
  for (unsigned OpIdx = 0; OpIdx != 2; ++OpIdx) {
    ...
    if (OpIdx < Desc.getNumOperands() &&
      OpInfos[OpIdx].OperandType == MCOI::OPERAND_MEMORY)
      HasOff = true;
  }
...
void Z80InstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  assert(DstReg.isPhysical() && SrcReg.isPhysical() &&
    ...
    if (Z80::G8RegClass.contains(DstReg, SrcReg)) {
      ...
      if (NeedEX) {
        ...
        if (MI == MBB.begin() || std::prev(MI)->getOpcode() != ExOpc) {
          ...
          markUseUndef(*Ex, Is24Bit ? Z80::UDE : Z80::DE, TRI);
          markUseUndef(*Ex, Is24Bit ? Z80::UHL : Z80::HL, TRI);
        } else
      ...
      if (NeedEX) {
        MachineInstr *Ex = BuildMI(MBB, MI, DL, get(ExOpc));
        markUseUndef(*Ex, Is24Bit ? Z80::UHL : Z80::HL, TRI);
      }
    }
    return;
    ...
    if (KillSrc && canExchange(DstReg, SrcReg)) {
      MachineInstrBuilder MIB = BuildMI(MBB, MI, DL,
                                        get(Is24Bit ? Z80::EX24DE
                                                    : Z80::EX16DE));
      markUseKill(*MIB, SrcReg, TRI);
      markDefDead(*MIB, SrcReg, TRI);
      markUseUndef(*MIB, DstReg, TRI);
      return;
    }
...
static Register scavengeOrCreateRegister(const TargetRegisterClass *RC,
  ...
  return RS ? RS->scavengeRegisterBackwards(*RC, II, false, SPAdj, AllowSpill)
...
void Z80InstrInfo::rewriteFrameIndex(MachineInstr &MI, unsigned FIOperandNum,
  ...
  if ((Opc == Z80::LEA24ro &&
    ...
    if (SaveFlags)
      markUseUndef(applySPAdjust(*BuildMI(MBB, II, DL,
                                         get(Is24Bit ? Z80::PUSH24AF
                                                     : Z80::PUSH16AF))),
                   Z80::AF, TRI);
...
bool Z80InstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  // Helper: check pointwise liveness of a physical register at a specific location.
  auto isLive = [&](MCRegister R, MachineBasicBlock::iterator Where) {
    return MBB.computeRegisterLiveness(&getRegisterInfo(), R, Where) !=
           MachineBasicBlock::LQR_Dead;
  };
  ...
  case Z80::Cmp24a0: {
    ...
    if (Opc == Z80::Cmp16a0) {
      // For 16-bit: LD A, H; OR A, L
      MCRegister HLReg = Reg;
      BuildMI(MBB, MI, DL, get(Z80::LD8gg), Z80::A)
          .addReg(RI.getSubReg(HLReg, Z80::sub_high));
      MI.setDesc(get(Z80::OR8ar));
      MIB.addReg(RI.getSubReg(HLReg, Z80::sub_low))
          .addReg(Z80::A, RegState::ImplicitDefine)
          .addReg(Z80::F, RegState::ImplicitDefine);
    } else {
      // For 24-bit, fall back to original method for now
      // TODO: Implement efficient 24-bit zero check
      MCRegister UndefReg = Z80::UBC;
      BuildMI(MBB, MI, DL, get(Z80::ADD24ao), Reg)
          .addReg(Reg).addReg(UndefReg, RegState::Undef);
      expandPostRAPseudo(*BuildMI(MBB, MI, DL, get(Z80::RCF)));
      MI.setDesc(get(Z80::SBC24ao));
  ...
  case Z80::LD8rp: {
    ...
    if (Z80::I8RegClass.contains(DstOp.getReg())) {
      ...
      if (emitPushPop) {
        markUseUndef(applySPAdjust(*BuildMI(MBB, MI, DL,
                                            get(Is24Bit ? Z80::PUSH24AF
                                                        : Z80::PUSH16AF))),
                      Z80::AF, TRI);
      }
...
bool Z80InstrInfo::optimizeCompareInstr(MachineInstr &CmpInstr, Register SrcReg,
  ...
  if (CpOpc) {
    int DeadDef =
        CmpInstr.findRegisterDefOperandIdx(Z80::A, &TRI, /*isDead*/true);
    if (DeadDef == -1)
      return false;
